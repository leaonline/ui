...
describe('ClozeItemTokenizer', () => {
  describe(getTokenValueForFlavor.name, () => {
    it('throws on undefined flavour', () => {
      ;[-1, 0, 5, true, false, {}, undefined, null].forEach((flavor) => {
        expect(() => getTokenValueForFlavor(flavor)).to.throw(
          `Unexpected flavor: ${flavor}`,
        )
      })
    })
  })
  describe(tokenizeBlanks.name, () => {
    it('it splits a blanks value into the correct tokens', () => {
      const flavor = '99'
    ...
    it('it splits a empty value into the correct tokens', () => {
      const flavor = '99'
  ...
  describe(tokenizeText.name, () => {
    it('it splits a text value into the correct tokens', () => {
      const flavor = '99'
  ...
  describe(toTokens.name, () => {
    ...
    it('throws on unexpected flavour', () => {
      const flavour = Math.random().toString(16)
      expect(() => fn({ value: `${flavour}$foo$bar` })).to.throw(
        `Unexpected flavor - ${flavour}`,
      )
    })
    it('allows to map splits to renderable tokens', () => {
      ...
      expect(fn({ value: 'noseparator' })).to.deep.equal({
        itemId,
        value: 'noseparator',
      })
    ...
    it('supports options but optional', () => {
      ...
      // multiple split by &
      expect(
        fn({ value: 'blanks$foo$bar$color=primary&border=dark' }),
      ).to.deep.equal({
        itemId,
        flavor: 2,
        isBlock: false,
  ...
  describe(ClozeItemTokenizer.tokenize.name, () => {
    let itemId
    beforeEach(() => {
      itemId = Random.id(6)
    })
    it('tokenizes a default cloze text correctly', () => {
...
{{blanks$[L]ara$Lara}} ist {{blanks$[h]eute$heute}} leider krank.`
    ...
    it('tokenizes a cloze text in table mode correctly', () => {
